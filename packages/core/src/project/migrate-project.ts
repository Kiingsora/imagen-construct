import {
  ...
  LEGACY_PROJECT_FORMAT_VERSION_V0_1_0,
  type Layer,
  type ProjectDocument,
  validateLegacyProjectDocumentV010,
  validateProjectDocument,
} from "@imagen-construct/contracts";
...
export function migrateProjectDocument(
  ...
  if (formatVersion === CURRENT_PROJECT_FORMAT_VERSION) {
    ...
    if (!result.valid) {
      throw new ProjectMigrationError(
        "Current project document failed validation.",
        result.issues,
      );
    }
    return result.value;
  ...
  if (formatVersion !== LEGACY_PROJECT_FORMAT_VERSION_V0_1_0) {
    throw new UnsupportedProjectFormatError(formatVersion);
  }

  const legacy = validateLegacyProjectDocumentV010(input);
  if (!legacy.valid) {
    throw new ProjectMigrationError(
      "Legacy project document failed validation before migration.",
      legacy.issues,
    );
  }
  ...
  const layers: Layer[] = legacy.value.layers
    ...
    .map(({ layer }) => {
      const { zIndex: _zIndex, ...current } = layer;
      return current;
    });
  ...
  return assertProjectDocument({
    ...
    createdAt: legacy.value.createdAt ?? timestamp,
    updatedAt: timestamp,
    layers,
  });
}
